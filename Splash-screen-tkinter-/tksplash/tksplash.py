...
class Tksplash:
    ...
    def add_splash_text(self, text, width, height):
        ...
        self.win.after(self.time, destroy)  # Destroying the label after the given time period
    ...
    def add_image_text(self, text, width, height, img, img_x, img_y, place_x, place_y, ):
        ...
        img_canvas = Canvas(self.win, width=img_x, height=img_y)
        img_canvas.place(x=place_x, y=place_y, anchor=CENTER)
        image = PhotoImage(file=img)
        img_canvas.create_image(place_x, place_y, anchor=NW, image=image)
        fontStyle = font.Font(family=self.text, size=self.size)
        splash_screen_label = Label(self.win, text=str(text), font=fontStyle, fg=self.colour)
        x_pos = width / 2  # finding the middle of the screen
        y_pos = height / 2  # finding the middle of the screen
        splash_screen_label.place(x=x_pos, y=y_pos, anchor=CENTER)
        self.win.after(self.time, destroy)
        self.win.after(self.time, destroy2)
        self.win.wait_window(splash_screen_label)
        # self.win.wait_window(img_canvas)
